...
const ReagrupamentoConjuge = ({ selectedPerson, onBack, onSuccess }) => {
  ...
  const {
    ...
    uploadService, 
    'Reagrupamento Familiar - Cônjuge', 
  ...
  const gerarProcessId = async () => {
    ...
    try {
      ...
      const novoId = `ReagrupamentoConjuge-${timestamp}-${randomHex}`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const dadosProcesso = {
        ...
        tipoDocumento: 'ReagrupamentoConjuge',
  ...
  const canSubmit = () => {
    return uploadState.documents.spouseTRFront?.uploaded && 
           uploadState.documents.spouseTRBack?.uploaded && 
           uploadState.documents.otherSpousePassport?.uploaded && 
      ...
      <Paper elevation={3} sx={{ p: 3, borderRadius: 2, backgroundColor: 'white' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
          <Typography variant="h5" sx={{ flexGrow: 1 }}>
            ...
            Reagrupamento Familiar - Através do Cônjuge
          </Typography>
        </Box>
        ...
        <Typography variant="body1" paragraph sx={{ color: 'text.secondary' }}>
          Por favor, carregue os documentos necessários para o reagrupamento familiar através do cônjuge.
        </Typography>
        
        <Grid container spacing={2}>
          <Grid item xs={12} md={6}>
            <DocumentUploader
              label="TR ou CC do Cônjuge (frente)"
              required={true}
              onFileSelected={(file) => handleFileSelected('spouseTRFront', file)}
              fileUploaded={uploadState.documents.spouseTRFront?.uploaded}
              error={uploadState.documents.spouseTRFront?.error}
              errorMessage={uploadState.documents.spouseTRFront?.errorMessage}
            />
          </Grid>
          
          <Grid item xs={12} md={6}>
            <DocumentUploader
              label="TR ou CC do Cônjuge (verso)"
              required={true}
              onFileSelected={(file) => handleFileSelected('spouseTRBack', file)}
              fileUploaded={uploadState.documents.spouseTRBack?.uploaded}
              error={uploadState.documents.spouseTRBack?.error}
              errorMessage={uploadState.documents.spouseTRBack?.errorMessage}
            />
          </Grid>
          
          <Grid item xs={12}>
            <DocumentUploader
              label="Passaporte do Outro Cônjuge"
              required={true}
              onFileSelected={(file) => handleFileSelected('otherSpousePassport', file)}
              fileUploaded={uploadState.documents.otherSpousePassport?.uploaded}
              error={uploadState.documents.otherSpousePassport?.error}
              errorMessage={uploadState.documents.otherSpousePassport?.errorMessage}
            />
          </Grid>
...
export default ReagrupamentoConjuge; 