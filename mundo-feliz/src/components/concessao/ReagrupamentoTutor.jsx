...
const ReagrupamentoTutor = ({ selectedPerson, onBack, onSuccess }) => {
  ...
  const gerarProcessId = async () => {
    ...
    try {
      ...
      const novoId = `ReagrupamentoTutor-${timestamp}-${randomHex}`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (processResult.extractedData && processResult.extractedData.gpt && 
          processResult.extractedData.gpt.pessoaQueRegrupa) {
        processResult.extractedData.gpt.pessoaQueRegrupa.parentesco = "TUTOR";
        
        if (processResult.extractedData.campos && processResult.extractedData.campos.pessoaQueRegrupa) {
          processResult.extractedData.campos.pessoaQueRegrupa.parentesco = "TUTOR";
        }
      }
      
      const uploadResult = await submitDocuments({
        tipoProcesso: 'Reagrupamento Familiar - Tutor',
        tipoDocumento: 'ReagrupamentoTutor',
  ...
  const canSubmit = () => {
    return uploadState.documents.tutorTRFront?.uploaded && 
           uploadState.documents.tutorTRBack?.uploaded && 
           uploadState.documents.minorPassport?.uploaded && 
      ...
      <Paper elevation={3} sx={{ p: 3, borderRadius: 2, backgroundColor: 'white' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
          <Typography variant="h5" sx={{ flexGrow: 1 }}>
            <SupervisorAccountIcon sx={{ mr: 1, verticalAlign: 'middle', color: '#1976d2' }} />
            Reagrupamento Familiar - Através do Tutor
          </Typography>
        </Box>
        ...
        <Typography variant="body1" paragraph sx={{ color: 'text.secondary' }}>
          Por favor, carregue os documentos necessários para o reagrupamento familiar através do tutor.
        </Typography>
        
        <Grid container spacing={2}>
          <Grid item xs={12} md={6}>
            <DocumentUploader
              label="TR ou CC do Tutor (frente)"
              required={true}
              onFileSelected={(file) => handleFileSelected('tutorTRFront', file)}
              fileUploaded={uploadState.documents.tutorTRFront?.uploaded}
              error={uploadState.documents.tutorTRFront?.error}
              errorMessage={uploadState.documents.tutorTRFront?.errorMessage}
            />
          </Grid>
          
          <Grid item xs={12} md={6}>
            <DocumentUploader
              label="TR ou CC do Tutor (verso)"
              required={true}
              onFileSelected={(file) => handleFileSelected('tutorTRBack', file)}
              fileUploaded={uploadState.documents.tutorTRBack?.uploaded}
              error={uploadState.documents.tutorTRBack?.error}
              errorMessage={uploadState.documents.tutorTRBack?.errorMessage}
            />
          </Grid>
          ...
          <Grid item xs={12}>
            <DocumentUploader
              ...
              required={true}
              onFileSelected={(file) => handleFileSelected('minorPassport', file)}
              fileUploaded={uploadState.documents.minorPassport?.uploaded}
              error={uploadState.documents.minorPassport?.error}
              errorMessage={uploadState.documents.minorPassport?.errorMessage}
            />
          </Grid>
          
          <Grid item xs={12} md={6}>
            <DocumentUploader
              label="Documento de Tutela (opcional)"
              required={false}
              onFileSelected={(file) => handleFileSelected('tutelageDocument', file)}
              fileUploaded={uploadState.documents.tutelageDocument?.uploaded}
              error={uploadState.documents.tutelageDocument?.error}
              errorMessage={uploadState.documents.tutelageDocument?.errorMessage}
            />
          </Grid>
...
export default ReagrupamentoTutor; 