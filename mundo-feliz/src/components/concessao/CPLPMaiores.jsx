...
const CPLPMaiores = ({ selectedPerson, onBack, onSuccess }) => {
  ...
  const gerarProcessId = async () => {
    ...
    try {
      ...
      const novoId = `CPLPMaiores-${timestamp}-${randomHex}`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const uploadResult = await submitDocuments({
        tipoProcesso: 'CPLPMaiores',
        tipoDocumento: 'CPLPMaiores',
      ...
      <Paper elevation={3} sx={{ p: 3, borderRadius: 2, backgroundColor: 'white' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
          <Typography variant="h5" sx={{ flexGrow: 1 }}>
            <PublicIcon sx={{ mr: 1, verticalAlign: 'middle', color: '#1976d2' }} />
            CPLP - Maiores
          </Typography>
        </Box>
        ...
        <Typography variant="body1" paragraph sx={{ color: 'text.secondary' }}>
          Por favor, carregue os documentos necessários para o processo CPLP para maiores.
        </Typography>
