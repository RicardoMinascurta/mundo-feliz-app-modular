...
const RenovacaoUniaoEuropeia = ({ selectedPerson, onBack, onSuccess }) => {
  ...
  const gerarProcessId = async () => {
    if (!processId) {
      ...
      try {
        ...
        const novoId = `RenovacaoUniaoEuropeia-${timestamp}-${randomHex}`;
  ...
  const canSubmit = () => {
    ...
    return uploadState.documents.passaporteEuropeu?.uploaded && 
           uploadState.documents.comprovativoTrabalho?.uploaded && 
      ...
      <Paper elevation={3} sx={{ p: 3, borderRadius: 2, backgroundColor: 'white' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
          <Typography variant="h5" sx={{ flexGrow: 1 }}>
            <EuroIcon sx={{ mr: 1, verticalAlign: 'middle', color: '#1976d2' }} />
            Renovação - União Europeia
          </Typography>
        </Box>
        ...
        <Typography variant="body1" paragraph sx={{ color: 'text.secondary' }}>
          Por favor, carregue os documentos necessários para renovação de autorização de residência para cidadãos da União Europeia.
        </Typography>
        
        <Grid container spacing={2}>
          <Grid item xs={12} md={6}>
            <DocumentUploader
              label="Passaporte Europeu"
              required={true}
              onFileSelected={(file) => handleFileSelected('passaporteEuropeu', file)}
              fileUploaded={uploadState.documents.passaporteEuropeu?.uploaded}
              error={uploadState.documents.passaporteEuropeu?.error}
              errorMessage={uploadState.documents.passaporteEuropeu?.errorMessage}
            />
          </Grid>

          <Grid item xs={12} md={6}>
            <DocumentUploader
              label="Comprovativo de Trabalho"
              required={true}
              onFileSelected={(file) => handleFileSelected('comprovativoTrabalho', file)}
              fileUploaded={uploadState.documents.comprovativoTrabalho?.uploaded}
              error={uploadState.documents.comprovativoTrabalho?.error}
              errorMessage={uploadState.documents.comprovativoTrabalho?.errorMessage}
            />
          </Grid>
...
export default RenovacaoUniaoEuropeia; 