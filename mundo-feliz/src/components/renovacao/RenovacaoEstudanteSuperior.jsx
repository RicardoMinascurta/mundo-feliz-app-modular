...
const RenovacaoEstudanteSuperior = ({ selectedPerson, onBack, onSuccess }) => {
  ...
  const gerarProcessId = async () => {
    if (!processId) {
      ...
      try {
        ...
        const novoId = `RenovacaoEstudanteSuperior-${timestamp}-${randomHex}`;
  ...
  const canSubmit = () => {
    ...
    return uploadState.documents.trFront?.uploaded && 
           ...
           uploadState.documents.declaracaoUniversidade?.uploaded && 
      ...
      <Paper elevation={3} sx={{ p: 3, borderRadius: 2, backgroundColor: 'white' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
          <Typography variant="h5" sx={{ flexGrow: 1 }}>
            ...
            Renovação - Estudante Ensino Superior
          </Typography>
        </Box>
        ...
        <Typography variant="body1" paragraph sx={{ color: 'text.secondary' }}>
          Por favor, carregue os documentos necessários para renovação de autorização de residência para estudante do ensino superior.
        </Typography>
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12}>
            <DocumentUploader
              label="Declaração da Universidade"
              required={true}
              onFileSelected={(file) => handleFileSelected('declaracaoUniversidade', file)}
              fileUploaded={uploadState.documents.declaracaoUniversidade?.uploaded}
              error={uploadState.documents.declaracaoUniversidade?.error}
              errorMessage={uploadState.documents.declaracaoUniversidade?.errorMessage}
            />
          </Grid>
...
export default RenovacaoEstudanteSuperior; 