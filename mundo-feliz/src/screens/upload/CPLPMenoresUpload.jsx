...
const CPLPMenoresUpload = () => {
  ...
  const {
    ...
    submitDocuments,
    processDocuments
  } = useUpload('cplp_CPLPMenor', processId, uploadService);
  ...
  const canSubmit = () => {
    ...
    const requiredDocs = ['passport', 'visa', 'fotoTipo', 'certificadoNascimento', 'autorizacaoParental'];
    ...
    <Container maxWidth="md" sx={{ py: 4 }}>
      <Paper elevation={3} sx={{ p: 3, borderRadius: 2, backgroundColor: 'white' }}>
        <Typography variant="h4" gutterBottom align="center" sx={{ mb: 3, color: '#1976d2' }}>
          Upload de Documentos - CPLP Menores
        </Typography>
        
        <Alert severity="info" sx={{ mb: 4 }}>
          <Typography variant="body1">
            Para processos CPLP Menores, são necessários documentos adicionais:
          </Typography>
          <ul>
            <li>Passaporte do menor</li>
            <li>Visto (se aplicável)</li>
            <li>Foto tipo passe recente</li>
            <li>Certidão de nascimento</li>
            <li>Autorização parental/representante legal</li>
          </ul>
        </Alert>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            <DocumentUploader
              ...
              required={true}
              onFileSelected={(file) => handleFileSelected('certificadoNascimento', file)}
              fileUploaded={uploadState.documents['certificadoNascimento']?.uploaded}
              error={uploadState.documents['certificadoNascimento']?.error}
              errorMessage={uploadState.documents['certificadoNascimento']?.errorMessage}
            />
          </Grid>
        </Grid>
        ...
        <Grid container spacing={3}>
          <Grid item xs={12}>
            <DocumentUploader
              ...
              required={true}
              onFileSelected={(file) => handleFileSelected('autorizacaoParental', file)}
              fileUploaded={uploadState.documents['autorizacaoParental']?.uploaded}
              error={uploadState.documents['autorizacaoParental']?.error}
              errorMessage={uploadState.documents['autorizacaoParental']?.errorMessage}
            />
          </Grid>
        </Grid>
...
export default CPLPMenoresUpload; 